...
//Modified from jquery.couch.js from CouchDB.
...
(function (root, factory) {
    if (typeof exports === 'object') {
        ...
        if (define.amd) 
            // AMD
            define([], function (b) {
                return (root.couchapi = factory(root.couchapi || {}, jQuery));
            });
        else
            //[bootstrapjs](//github.com/michieljoris/bootstrapjs)
            define({inject: [],
                    factory: function() {
                        return factory(root.couchapi || {}, jQuery);
                    }});
      
    } else {
        // Global variable
        root.couchapi = factory(root.couchapi || {}, jQuery);
    }
})(this, function(api, $) { 
    var uuidCache = [];
    ...
    api._async = {
        ...
        userDb : function(callback) {
            api._async.session({
                success : function(resp) {
                    var userDb = api._async.db(resp.info.authentication_db);
                    callback(userDb);
                },
                error : function(status, error, reason) {
                    callback(null, status, error, reason);
                }
            });
        },
        ...
        signup: function(user_doc, password, options) {      
            ...
            api._async.userDb(function(db, status, error, reason) {
                if (error) options.error(status, error, reason);
                else db.saveDoc(user_doc, options);
            });
        },
...
// console.log('couchapi_async loaded') ;
