//A simple vows based wrapper for couchapi_async.js, which is a modified
...
(function (root, factory) {
    if (typeof exports === 'object') {
        ...
        module.exports._async = require('./couchapi_async.js')._async;
        module.exports._pouch = require('./couchapi_pouch.js')._pouch;
    } else if (typeof define === 'function') {
        //neither amd nor bootstrap is tested as of March/14
        if (define.amd) 
            // AMD
            define(['lib/couchapi_async', 'lib/couchapi_pouch'], function (async, pouch) {
                return factory({ _async: async._async, _pouch: pouch._pouch });
            });
        else
            //[bootstrapjs](//github.com/michieljoris/bootstrapjs)
            define({ inject: ['lib/couchapi_async', 'lib/couchapi_pouch'],
                     factory: function(async, pouch) {
                       return factory({ _async: async._async, _pouch: pouch._pouch });
                     }});
      
    } else {
        // Global variable
        root.couchapi = factory(root.couchapi || {});
    }
    ...
    //##Couchdb
    // var api = {
        // _async: { ... } //attached by couchapi_async.js
    ...
    //###init
    //Couchapi will try use couchapi_async is present and url is passed
    //in. Otherwise will fall back to pouch.
    api.init = function(url, verbose) {
        couch = (api._async && url) ? api._async : api._pouch;
        if (!couch)
            throw new Error('couchapi can\'t talk to a database. Either load couchapi_async or couchapi_pouch, or both');
        couch.urlPrefix = url;
        if (verbose === 'verbose' || url === 'verbose' )
            api.info().when(
    ...
    api.replicationAdd = function(id, repDoc, sourceDesignDoc) {
        repDoc._id = id || api.UUID();
        if (repDoc.role)
            repDoc.user_ctx = { "roles": [repDoc.role] };
        if (repDoc.filterName)
            repDoc.filter = sourceDesignDoc + '/' + repDoc.filterName;
