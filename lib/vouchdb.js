//A simple vows based wrapper for vouchdb_async.js, which is a modified
...
(function (root, factory) {
    ...
    //##Couchdb
    // var api = {
    // _async: { ... } //attached by vouchdb_async.js
    ...
    //###init
    //Vouchdb will try use vouchdb_async if present and url is passed
    //in. Otherwise will fall back to pouch.
    api.init = function(url) {
        couch = (api._async && url) ? api._async : api._pouch;
        if (!couch)
            throw new Error('vouchdb can\'t talk to a database. Either load vouchdb_async or vouchdb_pouch, or both');
        couch.urlPrefix = url;
    };
